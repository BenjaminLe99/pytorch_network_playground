...
#         restframe_m = torch.sqrt(torch.clamp(restframe_vecs[..., E]**2 - restframe_p**2, min=self.eps))  # (B, M)
#         gamma = restframe_vecs[..., E] / (restframe_m + self.eps)  # (B, M)
#         beta = restframe_p / (restframe_vecs[..., E] + self.eps)  # (B, M)
#         beta_vecs = restframe_vecs[..., PX:] / (restframe_vecs[..., E, None] + self.eps)  # (B, M, 3)
#         n_vecs = beta_vecs / (beta[..., None] + self.eps)  # (B, M, 3)
...
#                 f = get("p2")**0.5
#             elif feature == "eta":
#                 ratio = torch.atanh(torch.clamp(get("pz") / (get("p") + self.eps), min=-1 + self.eps, max=1 - self.eps))
#                 f = torch.atanh(ratio)
#             elif feature == "phi":
#                 f = torch.atan2(get("py"), get("px"))
#             elif feature == "m":
#                 f = torch.sqrt(torch.clamp(torch.maximum(get("e")**2, get("p2") - get("p")), min=self.eps))
#             elif feature == "pair_cos":
#                 boosted_pvecs = boosted_vecs[..., PX:]  # (B, M, 3)
#                 boosted_p = get("p")
#                 f = ((boosted_pvecs @ boosted_pvecs.transpose(1, 2)) / ((boosted_p[..., None] + self.eps) @ (boosted_p[:, None, :] + self.eps))).flatten(start_dim=1)[..., self.lower_tril_indices]  # (B, (M**2-M)/2)
...
#                 f = torch.sqrt(boosted_dphi**2 + boosted_deta**2 + self.eps)
#             else:
...
class LBN(torch.nn.Module):
    ...
    def forward(
        ...
        particle_p = torch.sqrt(torch.clamp(torch.sum(particle_pvecs**2, dim=-1), min=0) + self.eps)  # (B, M)
        ...
        restframe_p = torch.sqrt(torch.clamp(torch.sum(restframe_pvecs**2, dim=-1), min=0) + self.eps)  # (B, M)
        ...
        # create boost objects
        restframe_m = torch.sqrt(torch.clamp((restframe_vecs[..., E]**2 - restframe_p**2), min=0) + self.eps)  # (B, M)
        gamma = restframe_vecs[..., E] / (restframe_m + self.eps)  # (B, M)
        beta = restframe_p / (restframe_vecs[..., E] + self.eps) # (B, M)
        beta_vecs = restframe_vecs[..., PX:] / (restframe_vecs[..., E, None] + self.eps)  # (B, M, 3)
        n_vecs = beta_vecs / (beta[..., None] + self.eps)  # (B, M, 3)
        ...
        def get(feature: str) -> torch.Tensor:
            ...
            if feature == "pt2":
                ...
                f = torch.sqrt(torch.clamp(get("pt2"), min=0) + self.eps)
            elif feature == "p2":
                f = get("pt2") + get("pz")**2
            elif feature == "p":
                f = torch.sqrt(torch.clamp(get("p2"), min=0) + self.eps)
            elif feature == "eta":
                f = torch.atanh(torch.clamp(get("pz") / get("p"), min=self.eps - 1.0, max=1.0 - self.eps))
            elif feature == "phi":
                f = torch.atan2(get("py"), get("px"))
            elif feature == "m":
                f = torch.sqrt(torch.clamp((torch.maximum(get("e")**2, get("p2")) - get("p")), min=0) + self.eps)
            elif feature == "pair_cos":
                boosted_pvecs = boosted_vecs[..., PX:]  # (B, M, 3)
                boosted_p = get("p")
                f = (
                ...
                boosted_deta = boosted_eta[..., None] - boosted_eta[:, None, :]  # (B, M, M)
                boosted_deta = boosted_deta.flatten(start_dim=1)[..., self.lower_tril_indices]  # (B, (M**2-M)/2)
                f = torch.sqrt(torch.clamp((boosted_dphi**2 + boosted_deta**2), min=0) + self.eps)
            else:
                raise RuntimeError(f"unknown feature '{feature}'")
            # cache and return
            cache[feature] = f
